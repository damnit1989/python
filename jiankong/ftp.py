...
def checkPort():
    ...
    sk.settimeout(1)
    try:
        sk.connect(('192.168.5.13',21))
        data['timestamp']=int(time.time())
        data['score']=0
        data['num'] = NUM
        data['reason'] = 'Server port 21 connect!'
        ...
        data['score']=20
        data['num'] = NUM
        data['reason'] = 'Server port 21 not connect!'
...
def checkVul():
    ...
    try:
        ftp=ftplib.FTP()
        ftp.connect('192.168.5.13',21)
        ftp.login('admin','admin')
        ftp.quit()
        data['timestamp']=int(time.time())
        data['score']=0
        data['num'] = NUM
        data['reason'] = '192.168.5.13-Password has not been modified!'
        data['type']  = 1  #扣分类型 1加分2减分
        data['signature'] = checkkey()
        data['group_id']  = GroupId
        data['device_id'] = ServerID
        ...
        data['num'] = NUM
        data['repair'] = True
        data['reason'] = '192.168.5.13-Password has been modified!'
        data['type']  = 1  #扣分类型 1加分2减分
        data['signature'] = checkkey()
        data['group_id']  = GroupId
        data['device_id'] = ServerID
