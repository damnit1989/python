...
def checkVul():
    #以下是检测程序的文件包含漏洞是否修补
    tx=requests.get('http://192.168.2.12/upload_file.php',allow_redirects=False).status_code
    if tx!=200:
        ...
        files={
            'uploaded': ('xxd2343dc69h.php', open('oa.php', 'rb'), 'image/jpeg'),
            'MAX_FILE_SIZE':(None,'100000'),
            'path':(None,'./data/uploadfile/'),
            'Upload':(None,'Upload')
        }
        headers ={
            'User-Agent': 'Mozilla/5.0 (Windows NT 6.3; rv:39.0) Gecko/20100101 Firefox/39.0',
            }
        req=requests.post('http://192.168.2.12/upload_file.php', files=files, headers=headers,allow_redirects=False).text
        url= re.findall(re.compile(r'.*?(?=\ssuccesfully)'), req)
        if len(url) >1:
            url='http://192.168.2.12%s'%url[0]
            #print url
            rex= requests.get(url,allow_redirects=False).status_code
            if rex==200:
                data['timestamp']=int(time.time())
                data['score']=0
                data['num'] = NUM
                data['reason'] = '192.168.2.12-oa2 upload not repair'
                data['type']  = 1  #扣分类型 1加分2减分
                data['signature'] = checkkey()
                data['group_id']  = GroupId
                data['device_id'] = ServerID
                ...
                data['num'] = NUM
                data['repair'] = True
                data['reason'] = '192.168.2.12-oa2 upload repair'
                data['type']  = 1  #扣分类型 1加分2减分
                data['signature'] = checkkey()
                data['group_id']  = GroupId
                data['device_id'] = ServerID
